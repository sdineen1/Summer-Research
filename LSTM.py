...
for i in range (time_steps, len(training_set_scaled)):
    X_train.append(training_set_scaled[i-time_steps:i, 7:16])   #training_set_scaled
    y_train.append(training_set_scaled[i, 7])                   #training_set_scaled
...
for i in range (time_steps , len(inputs)):
    X_test.append(inputs[i-time_steps:i, 7:16])
    
X_test = np.array(X_test)
y_test = inputs[time_steps:len(inputs),7]
